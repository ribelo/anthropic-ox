...
//         tool::{AnyTool, Tool, ToolBox, ToolError, ToolMetadataInfo, ToolResult, ToolUseBuilder},
...
//     futures::channel::mpsc::UnboundedSender<Result<ChatEventMessage, ApiError>>;
//
...
//         let tool_uses = response.tool_uses_owned();
...
//                         let message = MultimodalContent::text(
//                             serde_json::to_string(&tool_use.input).map_err(AgentError::from)?,
//                         );
//                         let result = subagent
...
//                     } else {
//                         Err(AgentError::UnknownTool(tool_use.name))
//                     }
//                 }
//             })
//             .buffer_unordered(4)
...
//                 Err(e) => {
//                     if let Some(tx) = &tx {
//                         tx.unbounded_send(Err(e)).unwrap();
//                     }
//                     continue;
//                 }
//             };
...
//                             event: event.clone(),
//                         };
//                         tx.unbounded_send(Ok(content)).unwrap();
//                     }
//                 }
//             }
//         }
...
//     ) -> Result<Messages, AgentError> {
//         loop {
//             let stream = self.prepare_request(messages.clone()).stream().await;
//             let response = self
...
//     #[error("Agent return no messages")]
//     NoMessages,
//     #[error("Agent try to use unknown tool: {0}")]
//     UnknownTool(String),
// }
//
// #[cfg(test)]
